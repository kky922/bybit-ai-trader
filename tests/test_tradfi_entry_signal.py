...
def test_tradfi_allows_spy_bear_regime_when_filters_pass(monkeypatch):
    ...
    sig, reason = check_tradfi_entry("TESTCMD", "commodity", _ohlcv(closes), _ohlcv(closes), _ohlcv(spy_bear))
...
def test_tradfi_rejects_low_volume(monkeypatch):
    ...
    sig, reason = check_tradfi_entry("TESTCMD", "commodity", _ohlcv(closes), _ohlcv(closes), _ohlcv(closes))
